	...
	<view class="takeout">
		...
		<view class="condition-search bg-white" :style="[{ top: CustomBar + 'px' }]">
			...
			<view v-else class="input-wrap padding-lr flex align-center" style="height:100%">
				<uni-search-bar class="search-box" :radius="20" cancelButton="always" bgColor="#EBEBEB" @cancel="inputCancel" @confirm="inputConfirm" @input="inputChange"></uni-search-bar>
			</view>
		</view>

		<view class="takeout-list-wrapper padding-bottom-lg">
			<takeout-card v-if="list.length" :list="list" @toDetails="toDetails"></takeout-card>
...
export default {
	components: { takeoutCard, tuiTips, uniSearchBar, uniLoadMore },
	data() {
		return {
	...
	methods: {
		getList() {
			...
			this.$http.post('/api/takeout/store_takeout_list', params).then(res => {
				if (res.code == 0) {
