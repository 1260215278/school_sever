		...
		<view v-if="insert || show" class="uni-calendar__content" :class="{ 'uni-calendar--fixed': !insert, 'uni-calendar--ani-show': aniMaskShow }">
			...
			<view class="uni-calendar__header">
				<view class="uni-calendar__header-btn-box" @click="pre">
					...
					<view class="uni-calendar__header-btn"></view>
				</view>
				<text class="uni-calendar__header-text">{{ (nowDate.year || '') + '年' + (nowDate.month || '') + '月' }}</text>
				<view class="uni-calendar__header-btn-box" @click="next">
					<!-- <view class="uni-calendar__header-btn uni-calendar--right"></view> -->
					<view class="uni-calendar__header-btn-right"></view>
				</view>
			...
			<button @click="confirm" :disabled="!confirmDisabled" class="confirm-btn">确定</button>
		</view>

		<!-- <view class="calendar-confirm-btn"><button @click="calendarConfirm" :disabled="calendarConfirmDisabled" type="" class="cu-btn block bg-green lg">确定</button></view> -->
	</view>
</template>
...
export default {
	...
	methods: {
		...
		init(date) {
			...
			this.$set(this.calendar,'todayClick', false )
		},
		open() {
			this.show = true;
			this.$nextTick(() => {
		...
		choiceDate(weeks) {
			...
			this.weeks = this.cale.weeks;
			if (this.cale.multipleStatus.before && this.cale.multipleStatus.after&&this.cale.multipleStatus.before!=this.cale.multipleStatus.after) {
				this.confirmDisabled = true;
			} else {
				this.confirmDisabled = false;
			}
			if(weeks.isDay) {
				this.$set(this.calendar,'todayClick', true)
			} else {
				this.$set(this.calendar,'todayClick', false)
			}
			this.change();
...
.uni-calendar__header-btn {
	...
	border-right-width: 26upx;
	border-left-width: 0;
	border-right-color: #26d499;
	display: inline-block;
	vertical-align: middle;
}
.uni-calendar__header-btn-right {
	width: 0;
	height: 0;
	border: 16upx solid transparent;
	border-left-width: 26upx;
	border-right-width: 0;
	border-left-color: #26d499;
	display: inline-block;
	vertical-align: middle;
}
