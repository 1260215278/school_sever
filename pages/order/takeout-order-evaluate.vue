<template>
	<view class="takeout-order-evaluate">
		...
		<view class="container  padding-lr">
			...
			<view class="item flex align-center padding-sm text-df">
				<view class="margin-right">菜品味道</view>
				<rate :current="form.star" :size="20" active="#FFD200" @change="rateChange($event, 0)"></rate>
			</view>
			<view class="item item flex align-center padding-sm text-df">
				<view class="margin-right">商家服务</view>
				<rate :current="form.server_star" :size="20" active="#FFD200" @change="rateChange($event, 1)"></rate>
			</view>
			<view class="item item flex align-center padding-sm text-df">
				<view class="margin-right">配送服务</view>
				<rate :current="form.deliver_star" :size="20" active="#FFD200" @change="rateChange($event, 2)"></rate>
			</view>
			<view class="item item flex align-center padding-sm text-df">
...
export default {
	...
	computed: {
		...
		disabled() {
			return this.form.star && this.form.server_star && this.form.deliver_star;
		}
	},
	onLoad(option) {
	...
	methods: {
		...
		rateChange(e, index) {
			// index 0--菜品服务 1---商家服务 2--配送服务
			if (index === 0) {
				this.form.star = e.index;
			} else if (index === 1) {
				this.form.server_star = e.index;
			} else if (index === 2) {
				this.form.deliver_star = e.index;
			}
		},
		submitIssue() {
			...
			this.$http.post('/api/takeout/order_takeout_praise', this.form).then(res => {
				if (res.code == 0) {
		...
		toView() {
			uni.redirectTo({
				url: `/pages/home/takeout-details?id=${this.store_id}`
			});
		}
	}
};
